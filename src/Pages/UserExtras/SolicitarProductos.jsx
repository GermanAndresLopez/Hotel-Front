...
const mockItems = [
  {
    ...
    precio: "30.000",
    categoria: 'Producto',
    imagen: 'https://m.media-amazon.com/images/I/71K3vbBovIL._AC_UF1000,1000_QL80_.jpg',
    repetible: true
  },
  {
    id: 2,
...
export default function GuestProductServiceView() {
  ...
  return (
    <Container sx={{ mt: 10, mb: 5 }}>
      <Typography variant="h4" color="secondary" sx={{ fontFamily: 'monospace', mb: 4 }}>
        Productos y Servicios Disponibles
      </Typography>
      <Grid container spacing={4}>
        {mockItems.map((item) => (
          <Grid item xs={12} sm={6} md={4} key={item.id}>
            <Card sx={{ backgroundColor: '#191919', color: 'white', borderRadius: '16px' }}>
              <CardMedia component="img" height="140" image={item.imagen} alt={item.nombre} className="w-full h-40 object-cover rounded-t-lg" />
              <CardContent>
              ...
              <CardActions>
                <Button fullWidth variant="contained" color="primary" onClick={() => handleOpen(item)}>
                  Solicitar
                </Button>
              </CardActions>
            </Card>
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        ...
        >
          <Typography variant="h6" color="secondary" sx={{ fontFamily: 'monospace', mb: 2 }}>
            Confirmar Solicitud
          </Typography>
      ...
      <Box sx={{ mt: 6 }}>
        <Typography variant="h5" color="secondary" sx={{ fontFamily: 'monospace', mb: 2 }}>
          ...
          historial.map((item, idx) => (
            ...
            >
              ...
              <Typography variant="body2">
                {item.cantidad ? `Cantidad: ${item.cantidad}` : ''} {item.fecha ? `| Fecha: ${item.fecha}` : ''} {item.hora ? `| Hora: ${item.hora}` : ''}
              </Typography>
            </Box>
          ))
        )}
